...
public class PastryPanel extends JPanel implements ActionListener
{
	PastryData t;
	JPanel p[];
	JLabel image[];
	...
	public PastryPanel()
	{
		...
		{	
			...
			image[i] = new JLabel((ImageIcon)((t.PastryList).getValueAt(i,4)));
			b[i] = new JButton("ADD");
			name[i] = new JLabel((String)(t.PastryList).getValueAt(i,1));
			price[i] = new JLabel(((Double)((t.PastryList).getValueAt(i,2))).toString());
		   ...
		   for(int i=0;i<row;i++)
			{
				if(a.getSource() == b[i])
				{
					String name = (String)(t.PastryList).getValueAt(i,1);
					Double pro_Price = (Double)(t.PastryList).getValueAt(i,2);
