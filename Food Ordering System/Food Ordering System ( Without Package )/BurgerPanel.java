...
class BurgerPanel extends JPanel implements ActionListener
{
	BurgerData t;
	JPanel p[];
	JLabel image[];
	...
	public BurgerPanel()
	{
		...
		{	
			...
			image[i] = new JLabel((ImageIcon)((t.BurgerList).getValueAt(i,4)));
			b[i] = new JButton("ADD");
			name[i] = new JLabel((String)(t.BurgerList).getValueAt(i,1));
			price[i] = new JLabel(((Double)((t.BurgerList).getValueAt(i,2))).toString());
		   ...
		   for(int i=0;i<row;i++)
			{
				if(a.getSource() == b[i])
				{
					String name = (String)(t.BurgerList).getValueAt(i,1);
					Double pro_Price = (Double)(t.BurgerList).getValueAt(i,2);
